...
#%%
'''
All available columns in the data:
['Unnamed: 0', 'BGTJobId', 'JobDate', 'SOC', 'SOCName', 'Employer',
       'SIC1', 'SIC1Name', 'City', 'LAD', 'TTWA', 'County', 'Region_Nation',
       'Latitude', 'Longitude', 'MinDegree', 'MaxDegree', 'MinEdu', 'MaxEdu',
...
'''
#%%
# list some file names that are shared across scripts
saveoutput = '/Users/stefgarasto/Google Drive/Documents/results/BG/'
savelocaloutput = '/Users/stefgarasto/Local-Data/Results/SDS-BG/'
savefigures = os.path.join(saveoutput, 'figures/')
#filename= '/Users/stefgarasto/Local-Data/Burning-glass/Job_ads_2012_2018/
#{}_reliable_soc_ads.csv'
...
data_with_edu = os.path.join(saveoutput,'bgdata_with_edu.zip')
data_with_exp = os.path.join(saveoutput,'bgdata_with_exp.zip')
...
soccolnames = {1: 'Major Group', 2:'Sub-Major Group', 3: 'Minor Group',
               4: 'Unit   Group'}
# create dictionary of SOC codes at different levels to names
socnames_dict = {}
...
super_suites_colours = {'management': nesta_colours[1],
                        'engineering': nesta_colours[3],
                        'construction': nesta_colours[4],
                        'financialservices':nesta_colours[6]}
...
def load_all_bg_data(all_years = None, coi = None):
    t0 = time.time()
    filename= os.path.join('/Users/stefgarasto/Local-Data/Burning-glass/',
     'Job_ads_2012_2018/{}_reliable_soc_ads.csv')
    if not all_years:
    ...
    for ix,year in enumerate(all_years):
        if ix == 0:
            ...
            bgdata = pd.concat((bgdata,
                                pd.read_csv(filename.format(year))[coi]))
    print('Time needed to load all the data: {:.4f}s'.format(time.time()-t0))
    return bgdata
...
#%% to just compute the scores given targets and predictions
